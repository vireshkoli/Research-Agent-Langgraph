...
def test_reasoning_tokens_are_reported_but_not_billed_separately() -> None:
    ...
    # completion_tokens. Adding them again would silently inflate every cost number.
    call = llm._record(
        fake_response(1000, 0, 500, reasoning=400),
        model=NANO,
        purpose="act",
        elapsed_ms=1.0,
        batch=False,
...
def test_process_guard_fires_without_any_tracker(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(llm, "settings", lambda: SimpleNamespace(max_process_cost_usd=0.002))
    llm._reset_process_spend()
    try:
        llm._record(
            fake_response(1000, 0, 0),
            model=NANO,
            purpose="act",
            elapsed_ms=1.0,
            batch=False,
...
def test_process_guard_can_be_disabled_with_zero(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(llm, "settings", lambda: SimpleNamespace(max_process_cost_usd=0))
    llm._reset_process_spend()
    try:
        for _ in range(50):
            llm._record(
                fake_response(100_000, 0, 100_000),
                model=NANO,
                purpose="act",
                elapsed_ms=1.0,
                batch=False,
